...
To use a custom service account for Dataproc, follow the instructions [here for Dataproc](https://cloud.google.com/dataproc/docs/concepts/configuring-clusters/service-accounts#creating_a_cluster_with_a_user-managed_vm_service_account). Ensure that the custom service account for Dataproc has at minimum IAM permissions 1) `roles/storage.objectAdmin` over the GCS Bucket `$GCS_BUCKET_NAME` that will be used to stage the CloudDQ configs and executables and 2) `roles/bigquery.dataEditor` over the GCP BigQuery Dataset in the environment variable `CLOUDDQ_BIGQUERY_DATASET` that will be managed by CloudDQ. You will also need to update the `.scripts/dataproc-workflow-composer/dataproc_workflow_template.yaml` by adding `serviceAccount: <string_value>` under `placement.managedCluster.config.gceClusterConfig`,
...
export COMPOSER_ENVIRONMENT_NAME=clouddq-composer  # replace with a name that you want to use for Cloud Composer
```
...
```bash
sed -i s/\<your_gcp_project_id\>/${PROJECT_ID}/g cloud-data-quality/configs/entities/test-data.yml
sed -i s/dq_test/${CLOUDDQ_BIGQUERY_DATASET}/g cloud-data-quality/configs/entities/test-data.yml
```
...
bq load --project_id=${PROJECT_ID} --source_format=CSV --autodetect ${CLOUDDQ_BIGQUERY_DATASET}.contact_details cloud-data-quality/tests/data/contact_details.csv
```
...
### 7.1. Update variables in the [Dataproc Workflow Template YAML file](https://cloud.google.com/dataproc/docs/concepts/workflows/using-yamls) with your project configuration
...
### 7.2. Import the workflow template file to [Dataproc Workflow Templates](https://pantheon.corp.google.com/dataproc/workflows/templates)

```bash
export WORKFLOW_NAME=clouddq-workflow
...
### 7.3. Update variables in the DAG with your project configuration
...
sed -i s/\<template_id\>/${WORKFLOW_NAME}/g ${DAG_PY_FILE}
```

### 7.4. Copy DAG file to Composer Bucket
...
Once the Composer DAG execution is finished, you can see the DQ validation outcomes in the BigQuery table `dq_summary` by running:
