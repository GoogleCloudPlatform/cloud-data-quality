...
'<rule_binding_id>' AS _internal_rule_binding_id,
FROM
`<your-gcp-project-id>.<your_dataplex_zone_id>.asset_bucket` d
WHERE
contact_type = 'email'
),
last_mod AS (
...
'<your-gcp-project-id>.<your_dataplex_zone_id>.asset_bucket' AS _internal_table_id,
...
to_json(struct(data.contact_type,data.row_id,data.ts,data.value)) AS include_reference_columns_json,
FROM
zero_record
LEFT JOIN
data
