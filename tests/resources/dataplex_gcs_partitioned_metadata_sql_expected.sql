...
WITH
  ...
  FROM
    `<your-gcp-project-id>.<your_dataplex_zone_name>.__TABLES__` ),
  validation_results AS (
  SELECT
    CURRENT_TIMESTAMP() AS execution_ts,
    '<rule_binding_id>' AS rule_binding_id,
    'NO_DUPLICATES_IN_COLUMN_GROUPS' AS rule_id,
    '<your-gcp-project-id>.<your_dataplex_zone_name>.partitioned_gcs_asset' AS table_id,
    ...
    FALSE
  END
    AS complex_rule_validation_success_flag,
    """
  WITH
    zero_record AS (
    SELECT
    ...
    LEFT JOIN (
      ...
      FROM (
        ...
        USING
          (contact_type,
            value) ) custom_sql ) custom_sql_statement_validation_errors
    ON
  ...
  ORDER BY
    rule_id""" AS failed_records_query,
  FROM
    zero_record
  LEFT JOIN (
    SELECT
