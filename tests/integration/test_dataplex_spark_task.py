...
class TestSparkDataplexIntegration:

    @pytest.fixture
    def configs_archive_path(self, tmp_path):
        configs_archive_path = Path(tmp_path).joinpath("clouddq_spark_test_artifacts", "archives").absolute()
        configs_archive_path.mkdir(parents=True, exist_ok=True)
        return configs_archive_path

    @pytest.fixture
    def gcs_clouddq_configs_standard(self, temp_spark_configs_dir, configs_archive_path, gcs_bucket_name):
        ...
        with working_directory(configs_archive_path):
            ...
            shutil.make_archive('clouddq-configs', 'zip', temp_spark_configs_dir.parent, temp_spark_configs_dir.name)
            ...
            upload_blob(gcs_bucket_name, configs_file_path, f"test-artifacts/spark/{file_name}")
            gcs_uri = f"gs://{gcs_bucket_name}/test-artifacts/spark/{file_name}"
            print(gcs_uri)
            yield gcs_uri
    ...
    def gcs_clouddq_pyspark_driver(self, gcs_bucket_name):
        file_name = 'spark_clouddq_pyspark_driver.py'
        try:
            file_detail = importlib.util.find_spec(f"clouddq.integration.spark_cloudq_pyspark_driver")
            print(file_detail)
        except Exception as e:
            print(e)
        driver_path = file_detail.origin
        upload_blob(gcs_bucket_name, driver_path, f"test-artifacts/spark/{file_name}")
        gcs_uri = f"gs://{gcs_bucket_name}/test-artifacts/spark/{file_name}"
        print(gcs_uri)
        return gcs_uri
    ...
    def test_gcs_clouddq_pyspark_driver(self, gcs_bucket_name):
        file_name = 'test_spark_clouddq_pyspark_driver.py'
        try:
            file_detail = importlib.util.find_spec(f"clouddq.integration.test_spark_clouddq_pyspark_driver")
            print(file_detail)
        except Exception as e:
            print(e)
        driver_path = file_detail.origin
    ...
    def test_clouddq_zip_executable_paths(self,
        ...
        if gcs_clouddq_executable_path:
            gcs_zip_executable_name = f"{gcs_clouddq_executable_path}/spark/clouddq-executable.zip"
            gcs_zip_executable_hashsum_name = f"{gcs_clouddq_executable_path}/spark/clouddq-executable.zip.hashsum"
        else:
            ...
            upload_blob(gcs_bucket_name, clouddq_zip_build, "test-artifacts/spark/clouddq-executable.zip")
            gcs_zip_executable_name = f"gs://{gcs_bucket_name}/test-artifacts/spark/clouddq-executable.zip"
            ...
            upload_blob(gcs_bucket_name, hashsum_file, "test-artifacts/spark/clouddq-executable.zip.hashsum")
            gcs_zip_executable_hashsum_name = f"gs://{gcs_bucket_name}/test-artifacts/spark/clouddq-executable.zip.hashsum"
    ...
    def test_create_bq_dataplex_task(self,
        ...
        if test_driver:
            ...
            clouddq_pyspark_driver_filename: str = "test_spark_clouddq_pyspark_driver.py"
            ...
            clouddq_pyspark_driver_filename: str = "spark_clouddq_pyspark_driver.py"
        ...
        response = test_dq_dataplex_client.create_clouddq_task(
                    ...
                    enable_experimental_bigquery_entity_uris=False,
                    enable_experimental_pyspark_runner=True)
