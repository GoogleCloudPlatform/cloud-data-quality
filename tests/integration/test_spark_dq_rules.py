...
class TestDqRules:
    ...
    @pytest.fixture(scope="session")
    def create_spark_expected_results_table(
        self,
        client,
        test_resources,
        gcp_project_id,
        ...
        table_id = f"{gcp_project_id}.{target_bq_result_dataset_name}.spark_{target_bq_result_table_name}_expected"
        ...
        with open(test_resources / "spark_dq_rules_expected_results.json", "rb") as source_file:
    ...
    def test_spark_dq_rules(
        self,
        runner,
        temp_spark_configs_from_dq_rules_config_file,
        ...
        gcp_sa_key,
        create_spark_expected_results_table,
        source_spark_dq_rules_configs_file_path,
        test_resources,
        caplog,
    ):
        caplog.set_level(logging.INFO, logger="clouddq")
        try:
            temp_dir = Path(tmp_path).joinpath("clouddq_spark_test_dq_rules")
            ...
            with working_directory(temp_dir):
                ...
                dbt_path = dbt_runner.get_dbt_path(spark_runner=True)
                invocation_id = get_dbt_invocation_id(dbt_path)
                logger.info(f"Spark Dbt invocation id is: {invocation_id}")
                ...
                if len(rows):
                    logger.info(f"Input yaml from {source_spark_dq_rules_configs_file_path}:")
                    with open(source_spark_dq_rules_configs_file_path) as input_yaml:
                        lines = input_yaml.read()
                        logger.info(lines)
                    logger.warning(
                        "Rows with values not matching the expected "
                        "content in 'tests/resources/spark_expected_results.json':"
                    )
                    for row in rows:
