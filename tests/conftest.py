...
@pytest.fixture(scope="session")
def source_spark_dq_rules_configs_file_path():
    return Path("tests").joinpath("resources").joinpath("spark_dq_rules_configs.yml").absolute()
...
@pytest.fixture(scope="function")
def temp_spark_configs_from_dq_rules_config_file(
        gcp_project_id,
        gcp_dataplex_bigquery_dataset_id,
        gcp_dataplex_region,
        gcp_dataplex_lake_name,
        ...
        tmp_path):
    # Create temp directory
    temp_clouddq_dir = Path(tmp_path).joinpath("clouddq_spark_test_dq_rules_configs")
    # Copy over tests/resources/configs
    registry_defaults = shutil.copyfile(source_spark_dq_rules_configs_file_path, temp_clouddq_dir)
    # Prepare entity_uri configs
    with open(registry_defaults) as source_file:
        lines = source_file.read()
    with open(registry_defaults, "w") as source_file:
